...
# define JJ "\033[1;31m"
# define WD "\033[1;0m"
# define SHOW(...) std::cout << std::setw(10) CYAN << #__VA_ARGS__ << " == " << __VA_ARGS__ << std::endl
...
namespace ft {
	...
	public:
		...
		typedef typename ft::vector_iterator<T>					iterator;				// TODO: vector-um senc mi ban er grac
		typedef typename ft::vector_iterator<T>					const_iterator;
		typedef typename ft::rv_vector_iterator<iterator>		reverse_iterator;
		typedef typename ft::rv_vector_iterator<const_iterator>	const_reverse_iterator;
	...
	public:
		...
		// -- default constructor - //
		explicit vector(const allocator_type& __alloc = allocator_type()): _alloc(__alloc), _size(0), _capacity(0), _vptr(nullptr)
		{}
		...
		vector(InputIterator __start, InputIterator __end, const allocator_type& __alloc = allocator_type(),
			typename ft::enable_if<ft::is_integral<InputIterator>::value>::type* = 0):	_alloc(__alloc), _size(0)
		{
			InputIterator	it(__start);
			while (it++ != __end)
				++_size;
			
			_capacity = _size;
			_vptr = _alloc.allocate(_capacity);
			for (int i = 0; __start != _end; ++__start, ++i)
				_alloc.construct(&_vptr[i], *__start);
		}
		...
		//  replaces the contents with copeis of those in the range [first, last]  //
		template <class InputIterator>
		void	assign(InputIterator first, InputIterator last, typename ft::enable_if<!ft::is_integral<InputIterator>::value>::type* = 0)
		{
			clear();
			size_type n = static_cast<size_type>(last - first);		// TODO: maybe I have to write for this std::distance
			if (n > _capacity)
			{
				_alloc.deallocate(_vptr, _capacity);
		...
		reverse_iterator		begin() 		{ return reverse_iterator(_vptr + _size - 1); }
		const_reverse_iterator	begin() const 	{ return const_reverse_iterator(_vptr + _size - 1); } // TODO: not sure about -1, not tested yet
		reverse_iterator		end() 			{ return reverse_iterator(_vptr - 1); }
		const_reverse_iterator	end() const 	{ return const_reverse_iterator(_vptr - 1); }
		...
		void	resize(size_type __n, value_type __val = value_type())
		{
			...
			if (__n > _capacity)
				insert(_vptr + _size, n - _size, __val);
			_size = __n;
		}
