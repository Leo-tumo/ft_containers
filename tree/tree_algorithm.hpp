...
//                                                                        //
//             tree_algorithm.hpp                                         //
//             Created: 2022/12/31 20:12:08 by leo                        //
//             Made by: leo <leonidas.xaralambos@gmail.com>               //
//                                                                        //
// ********************************************************************** //

#ifndef TREE_ALGORITHM_HPP
#define TREE_ALGORITHM_HPP
...
inline bool
tree_is_left_child(NodePtr ptr) {
    return ptr == ptr->parent->left;
}

template <typename NodePtr>
NodePtr
tree_max(NodePtr ptr) {
...
template <typename NodePtr>
void
tree_rotate_right(NodePtr node) {
...
template <typename NodePtr>
void
tree_rotate_left(NodePtr& root, NodePtr node) {
    if (node == root) {
        root = node->right;
    }
...
template <typename NodePtr>
void
tree_rotate_right(NodePtr& root, NodePtr node) {
	if (node == root) {
		root = node->left;
	}
...
template <typename NodePtr>
inline bool
tree_node_is_black(NodePtr node) {
...
template <typename NodePtr>
void
tree_insert_fix(NodePtr root, NodePtr z) {
    z->is_black = z == root;
...
// This function should only be called to fix a double red node case
template <typename NodePtr>
void
tree_delete_fix(NodePtr root, NodePtr x_parent) {
    NodePtr x = NULL;
...
template <typename NodePtr>
void
tree_transplant_node(NodePtr pos, NodePtr& node) {
...
template <typename NodePtr>
void
tree_remove_node(NodePtr root, NodePtr target)
{
    NodePtr y = target;
