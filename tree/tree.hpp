...
//                                                                        //
//             tree.hpp                                                   //
//             Created: 2022/12/31 20:11:45 by leo                        //
...
public:
    tree(const value_compare& comp)
        : _alloc(node_allocator()), _value_alloc(allocator_type()),
          _comp(comp), _end_node(), _size(0) {
        _begin_iter = end_node();
    }

    tree(const tree& other)
        : _alloc(other._alloc), _value_alloc(other._value_alloc),
          _comp(other._comp), _end_node(), _size(0) {
        _begin_iter = end_node();
        insert(other.begin(), other.end());
    }

    tree(const value_compare& comp, const allocator_type& alloc)
        : _alloc(node_allocator()), _value_alloc(alloc),
          _comp(comp), _end_node(), _size(0) {
        _begin_iter = end_node();
    }
...
private:
    ...
    template <typename Key>
    pair<end_node_pointer, end_node_pointer>
	eq_range(const Key& key) const {
        node_pointer ptr = root();
    ...
    iterator
	insert_at(node_pointer& pos, end_node_pointer parent, const value_type& value) {
    ...
    node_pointer
	construct_node(const value_type& value) {
    ...
    template <typename Key>
    end_node_pointer
	find_pointer(const Key& key) const {
        node_pointer ptr = root();
        while (ptr != NULL) {
            if (value_comp()(key, ptr->value)) {
                ptr = ptr->left;
    ...
    template <typename Key>
    node_pointer&
	find_pos(end_node_pointer& parent, const Key& key) const 
    {
    ...
    template <typename Key>
    node_pointer&
	find_pos(iterator hint, end_node_pointer& parent, const Key& key,
    ...
    void
	delete_node(node_pointer node) {
		_value_alloc.destroy(&node->value);
        _alloc.deallocate(node, 1);
    }

    void
	destroy(node_pointer node)
    {
        if (node != NULL) {
            destroy(node->left);
            destroy(node->right);
